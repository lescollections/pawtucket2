 ...
 */
 	...
 	class StorymapController extends ActionController {
 		# -------------------------------------------------------
 		/**
 		 *
 		 *
 		...
 		public function __construct(&$po_request, &$po_response, $pa_view_paths=null) {
            ...
            $this->config = Configuration::load(__CA_THEME_DIR__.'/conf/storymap.conf');
 		...
 		public function __call($ps_function, $pa_args) {
 			...
 			if($ps_function == "index"){
				...
				MetaTagManager::setWindowTitle($this->request->config->get("app_display_name").": ".(($this->config->get('gallery_section_name')) ? $this->config->get('gallery_section_name') : _t("Gallery")));
 				$this->render("Storymap/index_html.php");
 			}else{
 				...
 				$this->render("Storymap/detail_html.php");
 			}
 		}
 		# -------------------------------------------------------
 		...
 		public function getSetItemRep(){
 			...
 			$this->render("Storymap/set_item_rep_html.php");
 		}
 		# -------------------------------------------------------
 		...
 		# -------------------------------------------------------
        public function getSetInfosJson() {
            ...
            $vn_c = 0;
            foreach($va_set_items as $vt_set_item) {
                ...
                if($t_object->getWithTemplate($this->config->get(object_georeference))) {
                    ...
                    $va_objects[$vn_object_id] = array(
                        ...
                        "title" => $t_object->getWithTemplate($this->config->get(object_title_template)),
                        "description" => $t_object->getWithTemplate($this->config->get(object_description_template)),
                        "latitude" => $coordinates[0],
                        "longitude" => $coordinates[1],
                        "date" => $t_object->getWithTemplate($this->config->get(object_date)),
                        "placename" => $t_object->getWithTemplate($this->config->get(object_placename)),
                        "zoom" => $this->config->get(zoom),
                        "line" => $this->config->get(line),
                        "media_credit" => $t_object->getWithTemplate($this->config->get(media_credit)),
                        "media_caption" => $t_object->getWithTemplate($this->config->get(media_caption))
                    );
                    //var_dump($t_object);
                }
